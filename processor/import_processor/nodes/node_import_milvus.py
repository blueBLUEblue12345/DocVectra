...
class NodeImportMilvus(BaseNode):
    ...
    def _step_2_prepare_collection(self, vector_dimension: int):
        ...
        # 2. 集合不存在则创建，存在则检查索引完整性
        collections_name = self.config.chunks_collection
        if milvus_client.has_collection(collections_name):
            # 检查稀疏向量索引是否存在，不存在则删除重建
            try:
                index_info = milvus_client.describe_index(collections_name, "sparse_inverted_index")
                if not index_info:
                    raise Exception("索引不存在")
            except Exception:
                self.logger.warning(f"集合 {collections_name} 索引不完整，删除重建")
    ...
    def _step_3_clean_old_data(self, client, chunks_json_data):
        ...
        if not client.has_collection(collection_name):
            raise MilvusError(f"集合 {collection_name} 不存在")
...
if __name__ == "__main__":
    ...
    json_path = r"E:\output\M7268系列用户手册\state_vector.json"
